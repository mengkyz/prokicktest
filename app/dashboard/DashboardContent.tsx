...
export default function DashboardContent({ userId }: Props) {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    let bookingQuery = supabase
      .from('bookings')
      .select(
        `*, classes (*, coaches(name), venues(name)), child_profiles(nickname), user_packages (id, package_templates (name))`,
      )
  ...
  const nextPackage = () => {
    if (currentPkgIdx < packages.length - 1) setCurrentPkgIdx((p) => p + 1);
  };
  const prevPackage = () => {
    if (currentPkgIdx > 0) setCurrentPkgIdx((p) => p - 1);
  };
  ...
  const handleDevBypass = async () => {
    ...
    const formData = new FormData();
    const dummyFile = new File(['dev_bypass'], 'dev_bypass.txt', { type: 'text/plain' });
  ...
  return (
    <div className={`min-h-screen bg-gray-50 flex justify-center ${kanit.className} overflow-hidden`}>
      {processing && (
          ...
          <div className="px-5 space-y-6 mt-2">
            ...
            {loading ? (
              ...
              <div className="relative">
                {packages.length > 1 && (
                  <>
                    <button onClick={prevPackage} disabled={currentPkgIdx === 0} className={`absolute left-0 top-1/2 -translate-y-1/2 -ml-2 z-10 p-1.5 rounded-full bg-white shadow-md border border-gray-100 text-gray-600 transition-opacity ${currentPkgIdx === 0 ? 'opacity-0 pointer-events-none' : ''}`}>
                      <ChevronLeft size={20} />
                    </button>
                    <button onClick={nextPackage} disabled={currentPkgIdx === packages.length - 1} className={`absolute right-0 top-1/2 -translate-y-1/2 -mr-2 z-10 p-1.5 rounded-full bg-white shadow-md border border-gray-100 text-gray-600 transition-opacity ${currentPkgIdx === packages.length - 1 ? 'opacity-0 pointer-events-none' : ''}`}>
                ...
                <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-lg transition-all duration-300 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start">
                      <h3 className="font-bold text-gray-800 text-lg tracking-tight">{activePackage.package_templates.name}</h3>
                      {packages.length > 1 && (
                    ...
                    <div className="mt-4 flex items-end justify-between">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[10px] text-gray-500 font-medium">
                          Extra: <span className={activePackage.extra_sessions_purchased >= 2 ? 'text-red-500' : 'text-blue-600'}>{activePackage.extra_sessions_purchased}</span>/2
                        </span>
                        {activePackage.extra_sessions_purchased < 2 && (
                          <button onClick={(e) => { e.stopPropagation(); initiateBuyExtra(activePackage); }} className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-lg font-bold hover:bg-indigo-100 transition-colors border border-indigo-100">
                            <span>+ Top-up</span>
                          </button>
                        )}
                      </div>
            ...
            {/* Book button */}
            <button
              onClick={() => router.push(`/book${activeProfileId ? `?childId=${activeProfileId}` : ''}`)}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-sm rounded-t-2xl sm:rounded-2xl p-6 shadow-2xl">
              ...
              <div className="space-y-3">
                ...
                {children.map((child) => (
                  <button key={child.id} onClick={() => handleSwitchProfile(child.id)} className={`w-full flex items-center gap-4 p-3 rounded-xl border transition-all ${activeProfileId === child.id ? 'border-[#1e2e5c] bg-blue-50' : 'border-gray-100 hover:bg-gray-50'}`}>
        ...
        {modal.isOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-6">
              <h3 className="text-xl font-bold mb-4 font-['Kanit'] text-gray-900">{modal.title}</h3>
                ...
                {pastBookings.length === 0 ? (
                  ...
                  pastBookings.map((b) => (
                    <div key={b.id} className="text-xs border border-gray-100 p-4 rounded-xl flex justify-between items-center">
                      <div>
                        <div className="font-bold text-gray-800 text-sm mb-0.5">{formatBookingDate(b.class_date)}</div>
                        <div className="text-gray-500">{b.classes.venues?.name || b.classes.location}</div>
                      </div>
                      <span className={`px-2.5 py-1 rounded-lg font-bold ${b.status === 'booked' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{b.status}</span>
                    </div>
                  ))
                )}
              </div>
...
const BookingCard = ({ booking, activePackage, onCancel, processing }: any) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden group">
      <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${isStandby ? 'bg-yellow-400' : 'bg-[#1e2e5c]'}`}></div>
      <div className="flex justify-between items-start mb-4 pl-2">
        <div>
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wide mb-1 inline-block ${isStandby ? 'bg-yellow-50 text-yellow-700' : 'bg-blue-50 text-blue-700'}`}>
            {isStandby ? 'Waitlist' : 'Confirmed'}
          </span>
          <h4 className="font-bold text-gray-800 text-base">{activePackage?.package_templates?.name || 'Football Session'}</h4>
        </div>
        {isStandby && (
          <div className="text-center bg-yellow-100 text-yellow-800 rounded-lg px-2 py-1">
            ...
            <span className="block text-lg font-black leading-none">#{booking.standby_order}</span>
          </div>
        )}
      </div>
